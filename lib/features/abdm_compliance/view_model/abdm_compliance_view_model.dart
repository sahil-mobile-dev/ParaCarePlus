...
class AbdmComplianceNotifier extends StateNotifier<AbdmComplianceState> {
  ...
  void _startSimulations() {
    _liveTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
      ...
      final newLatency = (state.apiPerformance.latencyMs +
              _random.nextInt(20) -
              10)
          .clamp(195, 245);
  ...
  void sortDistricts(String sortBy) {
    ...
    if (sortBy == 'District') {
      ...
      list.sort((a, b) => b.abhaCoveragePercent.compareTo(a.abhaCoveragePercent));
    } else if (sortBy == 'EMR') {
      ...
      list.sort((a, b) => b.ePrescriptionPercent.compareTo(a.ePrescriptionPercent));
    } else if (sortBy == 'S&S') {
      list.sort((a, b) => b.scanAndSharePercent.compareTo(a.scanAndSharePercent));
  ...
  Future<void> refresh() async {
    ...
    final updatedDistricts = state.districts.map((d) {
      ...
      return d.copyWith(
        abhaCoveragePercent:
            (d.abhaCoveragePercent + coverageOffset).clamp(40.0, 99.0),
        score: newScore,
        grade: newGrade,
      );
    }).toList();
